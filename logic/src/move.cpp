...
Move generate_move(int from, int to, int piece, int type, int castle_side) { // you dont have to specify piece if you are doing an en pessant
    Move out;
    out = from;
    out |= to << 6;
    out |= type << 12;
    if (type != CASTLE && type != EN_PESSANT) {
        out |= piece << 13;
    }
    else if (type == CASTLE) {
        out |= castle_side << 15;
